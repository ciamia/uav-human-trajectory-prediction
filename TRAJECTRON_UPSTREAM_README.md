...
#### Kendi object detection / video CSV'nizi kullanmak ####
Videodan çıkan insan koordinatları CSV ise, bunu Trajectron++'ın kullandığı `.pkl` formatına çevirmek için:

1. **CSV formatı:** Her satır bir frame'de bir kişi: `frame_id`, `track_id`, `pos_x`, `pos_y`. Kolon isimleri farklıysa script'e parametreyle verilir.
2. **PKL üretme** (evaluation veya training için):
```
cd experiments/pedestrians
python csv_to_trajectron_data.py --csv /yol/detections.csv --output ../processed/my_video_test.pkl
```
Kolon isimleri farklıysa: `--frame_col frame --track_col id --x_col x --y_col y --sep ","`
3. **Evaluation:** Önceden eğitilmiş bir modelle tahmin almak için `experiments/pedestrians` içinden:
```
python evaluate.py --model models/<model_klasoru> --checkpoint <epoch> --data ../processed/my_video_test.pkl --output_path results --output_tag my_video --node_type PEDESTRIAN
```
Detaylar ve ek seçenekler için `csv_to_trajectron_data.py` dosyasının başındaki docstring'e bakın.
