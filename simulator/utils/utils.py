...
from configs.configuration_parameters import DirectoryConfig as GPConfig
...
def unit_vec(q):
    """
    Normalizes a vector to be unit vector.
    :param q: 3-dimensional numpy array
    :return: the unit vector in the same data format as the original one
    """
    ...
    if isinstance(q, np.ndarray):
        ...
        q_norm = cs.sqrt(cs.sumsqr(q))
        # Use CasADi's symbolic expression for comparison
        # The condition q_norm < 1e-6 should be checked symbolically using `cs.lt`
        q_norm_less_than_threshold = cs.lt(q_norm, 1e-6)

        # Return the unit vector with symbolic conditional logic
        return cs.if_else(q_norm_less_than_threshold, q, 1 / q_norm * q)
...
def quaternion_state_mse(x, x_ref, mask):
    ...
    """
    if np.array(x).shape[0] == 13: # original, only for UAV
        mask = [1] * 9 + [0] * 3
        ...
        mask = [1] * 18 + [0] * 6
        q_error = q_dot_q(x[9:13], quaternion_inverse(x_ref[9:13]))
        e = np.concatenate((x[:3] - x_ref[:3], x[3:6] - x_ref[3:6],
                            x[6:9] - x_ref[6:9], q_error[1:], 
                            x[13:16] - x_ref[13:16], x[16:19] - x_ref[16:19],
                            x[19:22] - x_ref[19:22], x[22:25] - x_ref[22:25]))
...
def separate_variables(traj):
    ...
    """
    # print("Separating variables from trajectory with shape: ", traj.shape)
    # print("Separating variables from trajectory with shape[0]: ", traj.shape[0])
    if traj.shape[1] == 13:  # original, only for UAV
        ...
        dqc_traj = traj[:, 19:22]
        r_traj   = traj[:, 22:25]
        return [p_traj, pl_traj, qc_traj, a_traj, v_traj, vl_traj, dqc_traj, r_traj]
