...
def draw_drone_simulation(art_pack, x_trajectory, quad, targets, targets_reached, sim_traj=None,
    ...
    xs = x_trajectory[trajectory_start_pt:, 0]
    ys = x_trajectory[trajectory_start_pt:, 1]
    zs = x_trajectory[trajectory_start_pt:, 2]

    trajectories_artist.set_data(xs, ys)
    trajectories_artist.set_3d_properties(zs)
    ax.draw_artist(trajectories_artist)

    # Draw projected trajectory
    # Project onto Y-Z plane (fix X at x_min)
    proj_x0 = np.full_like(zs, ax.get_xlim()[0])
    projected_traj_artists[1].set_data(proj_x0, ys)
    projected_traj_artists[1].set_3d_properties(zs)

    # Project onto X-Z plane (fix Y at y_max)
    proj_y0 = np.full_like(zs, ax.get_ylim()[1])
    projected_traj_artists[0].set_data(xs, proj_y0)
    projected_traj_artists[0].set_3d_properties(zs)

    for projected_traj_artist in projected_traj_artists:
        ax.draw_artist(projected_traj_artist)
...
def cuav_trajectory_tracking_results(t_ref, x_ref, x_executed, u_ref, u_executed, title, w_control=None, legend_labels=None,
                                quat_error=True):

    if legend_labels is None:
        legend_labels = ['refer', 'sim_uav', 'sim_load']

    with_ref = True if x_ref is not None else False

    fig, ax = plt.subplots(3, 8, sharex='all', figsize=(14, 9))
    ...
    for i in range(3):
        ax[i, 0].plot(t_ref, x_executed[:, i], label=legend_labels[1])
        ax[i, off_set].plot(t_ref, x_executed[:, i+3], label=legend_labels[2])
        if with_ref:
            ax[i, 0].plot(t_ref, x_ref[:, i], label=legend_labels[0])
            ax[i, off_set].plot(t_ref, x_ref[:, i+3], label=legend_labels[0])   
        ax[i, 0].legend()
        ax[i, 0].set_ylabel(labels[i])

        ax[i, off_set].legend()
        ax[i, off_set].set_ylabel(labels[i])

    ax[0, 0].set_title(r'$p_v\:[m]$')
    ax[2, 0].set_xlabel(r'$t [s]$')

    ax[0, off_set].set_title(r'$p_l\:[m]$')
    ax[2, off_set].set_xlabel(r'$t [s]$')

    q_euler = np.stack([quaternion_to_euler(x_executed[j, 9:13]) for j in range(x_executed.shape[0])])
    for i in range(3):
        ax[i, 1].plot(t_ref, q_euler[:, i], label=legend_labels[1])
        ax[i, off_set + 1].plot(t_ref, x_executed[:, i + 6], label=legend_labels[2])
    if with_ref:
        ref_euler = np.stack([quaternion_to_euler(x_ref[j, 9:13]) for j in range(x_ref.shape[0])])
        q_err = []
        for i in range(t_ref.shape[0]):
            q_err.append(q_dot_q(x_executed[i, 3:7], quaternion_inverse(x_ref[i, 3:7])))
        q_err = np.stack(q_err)
        ...
        for i in range(3):
            ax[i, 1].plot(t_ref, ref_euler[:, i], label=legend_labels[0])
            ax[i, off_set + 1].plot(t_ref, x_ref[:, i + 6], label=legend_labels[0]) 
            if quat_error:
                ax[i, 1].plot(t_ref, q_err[:, i + 1], label='quat error')
    for i in range(3):
        ax[i, 1].legend()
        ax[i, off_set + 1].legend()

    ax[0, 1].set_title(r'$\theta\:[rad]$')
    ax[2, 1].set_xlabel(r'$t [s]$')
    ax[0, off_set + 1].set_title(r'$q_c\:-$')
    ax[2, off_set + 1].set_xlabel(r'$t [s]$')

    for i in range(3):
        ax[i, 2].plot(t_ref, x_executed[:, i + 13], label=legend_labels[1])
        ax[i, off_set + 2].plot(t_ref, x_executed[:, i + 16], label=legend_labels[2])   
        if with_ref:
            ax[i, 2].plot(t_ref, x_ref[:, i + 13], label=legend_labels[0])
            ax[i, off_set + 2].plot(t_ref, x_ref[:, i + 16], label=legend_labels[0])    
        ax[i, 2].legend()
        ax[i, off_set + 2].legend()

    ax[0, 2].set_title(r'$v_v\:[m/s]$')
    ax[2, 2].set_xlabel(r'$t [s]$')
    ax[0, off_set + 2].set_title(r'$v_l\:[m/s]$')
    ax[2, off_set + 2].set_xlabel(r'$t [s]$')

    for i in range(3):
        ax[i, 3].plot(t_ref, x_executed[:, i + 19], label=legend_labels[1])
        ax[i, off_set + 3].plot(t_ref, x_executed[:, i + 22], label=legend_labels[2])
        if with_ref:
            ax[i, 3].plot(t_ref, x_ref[:, i + 10], label=legend_labels[0])
            ax[i, off_set + 3].plot(t_ref, x_ref[:, i + 22], label=legend_labels[0])    
        if w_control is not None:
            ax[i, 3].plot(t_ref, w_control[:, i], label='control')
        ax[i, 3].legend()
        ax[i, off_set + 3].legend()

    ax[0, 3].set_title(r'$\omega\:[rad/s]$')
    ax[2, 3].set_xlabel(r'$t [s]$')
    ax[0, off_set + 3].set_title(r'$\dot{q}_c\:-$')
    ax[2, off_set + 3].set_xlabel(r'$t [s]$')
    ...
    img_save_dir = dir_path + '/../../results/images/'
    safe_mkdir_recursive(img_save_dir, overwrite=False)
    fig.savefig(img_save_dir + 'cuav_mse_exp', dpi=None, facecolor='w', edgecolor='w',
                orientation='portrait',
                transparent=False, pad_inches=0.1)
