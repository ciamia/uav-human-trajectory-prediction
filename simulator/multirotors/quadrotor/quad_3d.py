...
class Quadrotor3D:
    ...
    def __init__(self, noisy=False, drag=False, payload=False, motor_noise=False):
        ...
        :param slungload: Whether to simulate a cargo uav with a slungload in the simulation
        :type payload: bool
        ...
        # some threshold varaibles
        self.thre = 1e-6      # small threshold
    ...
    def f_vel(self, x, u, f_d):
        ...
        a_thrust = np.array([[0], [0], [np.sum(u)]]) / self.mv

        if self.drag:
            # Transform velocity to body frame
            v_b = v_dot_q(x[2], quaternion_inverse(x[1]))[:, np.newaxis]
            # Compute aerodynamic drag acceleration in world frame
            a_drag = -self.aero_drag * v_b ** 2 * np.sign(v_b) / self.mv
            # Add rotor drag
            a_drag -= self.rotor_drag * v_b / self.mv
        ...
        a_payload = -self.payload_mass * self.g / self.mv

        return np.squeeze(-self.g + a_payload + a_drag + v_dot_q(a_thrust + f_d / self.mv, angle_quaternion))
