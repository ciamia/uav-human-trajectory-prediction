...
from utils.utils import skew_symmetric, v_dot_q, quaternion_inverse
...
class Controller:
    def __init__(self, quad:Quadrotor3D, t_horizon=1, n_nodes=20,
        ...
        self.last_w = np.array([1e-5, 1e-5, 1e-5, 1e-5])
        ...
        for key, key_model in zip(acados_models.keys(), acados_models.values()):
            ...
            # \sum{(x - x_ref)^T Q (x - x_ref) + (u - u_ref)^T R (u - u_ref)} + terminal 
    ...
    def v_dynamics(self, rdrv_d):
        ...
        a_thrust = cs.vertcat(0.0, 0.0, f_thrust[0] + f_thrust[1] + f_thrust[2] + f_thrust[3]) / self.quad.mv
    ...
    def run_optimization(self, initial_state=None, goal=None, use_model=0, return_x=False, mode='pose'):
        ...
        # Set final condition
        if mode == "pose":
            goal_quat = np.asarray(goal[3:7] if len(goal) >= 7 else [1, 0, 0, 0], dtype=float)
            for j in range(self.N):
                y_ref = np.hstack([goal[:7], np.zeros(6), self.last_w])
                self.acados_ocp_solver[use_model].set(j, 'yref', y_ref)
            y_refN = np.array([goal[0], goal[1], goal[2], *goal_quat, 0,0,0, 0,0,0])
            self.acados_ocp_solver[use_model].set(self.N, 'yref', y_refN)
        else:
            for j in range(self.N):
                y_ref = np.hstack([goal[j,:], self.last_w])
                self.acados_ocp_solver[use_model].set(j, 'yref', y_ref)
            y_refN = np.array([goal[self.N,0], goal[self.N,1], goal[self.N,2], 
                               goal[self.N,3], goal[self.N,4], goal[self.N,5], goal[self.N,6], 
                               0,0,0, 0,0,0])
            self.acados_ocp_solver[use_model].set(self.N, 'yref', y_refN)
